...
register_markup = InlineKeyboardMarkup([
    [InlineKeyboardButton('Register', callback_data="init_register")]
])
...
status_main = InlineKeyboardMarkup([
    [InlineKeyboardButton('Own Soldier', callback_data="status_soldier_main"),
     InlineKeyboardButton('Own Equipment', callback_data="status_equipment_main")],
    [InlineKeyboardButton('UnSet Soldier', callback_data="status_quit_soldier_order")],
    ...
    for i in soldier_list:
        ...
        button_list.append(
            [InlineKeyboardButton(soldier_name,
             ...
             InlineKeyboardButton('Set Main Soldier',
                                  callback_data=f'status_select_soldier_order_{soldier_idx}')])

    button_list.append([InlineKeyboardButton('Back', callback_data="status_main")])
...
status_to_equip_main = InlineKeyboardMarkup([
    [InlineKeyboardButton('Back', callback_data="status_equipment_main")]]
)
...
def ranking_markup(page: int, limit: int, max_page: int, ranking: list[UserModel], chat_id: str):
    ...
    for i in ranking:
        ...
        if i.chat_id == chat_id:
            match_markup.append([InlineKeyboardButton(
                f'(Player) {rank} Place rating:{i.pvp_rating}, {i.wallet_address}',
                callback_data="asd")])
        else:
            match_markup.append([InlineKeyboardButton(
                f'{rank} Place rating:{i.pvp_rating}, {i.wallet_address}',
                callback_data="asd")])
    page_select = []
    if page != 1:
    ...
    match_markup.append([InlineKeyboardButton('Back', callback_data="init")])
